...
For the people, we can do this by taking the original affiliation matrix and pre-multiplying it by $\mathbf{D}p^{-1}$ (which now contains the \textit{inverse} of the first-order centralities of each person along the diagonals), yielding the row-stochastic matrix $\mathbf{P}_{pg}$ of dimensions $|P| \times |G|$:
...
We can do the same with the groups, except that we pre-multiply the \textit{transpose} of the original affiliation matrix by $\mathbf{D}g^{-1}$ (which now contains the \textit{inverse} of the first-order centralities of each group along the diagonals) yielding the matrix $\mathbf{P}_{gp}$ of dimensions $|G| \times |P|$:
...
Both $\mathbf{P}_{pp}$ and $\mathbf{P}_{gg}$ are square ($P \times |P|$ and $|G| \times |G|$) respectively) degree-normalized projection matrices linking people and groups. For the Southern Women data, they are shown in Figure~\ref{fig:dwproj}. 

It can be shown \citep{van2021correspondence}, that in the limit ($q \rightarrow \infty$), the iterative HH reflective centralities can be obtained as a solution of the eigensystem\footnote{More specifically since both equations~\ref{eq:dam1} and~\ref{eq:dam2} produce row stochastic matrices (row sums are fixed to sum to 1.0), their largest (dominant) eigenvalue will also be 1.0, which will be associated with a trivial first eigenvector containing constant values for both the people and the groups. Accordingly, the scores we seek, which are the same as the CA scores and the HH reflective centrality scores, are those stored in the eigenvectors associated with the \textit{second} largest eigenvalue~\citep{van2021correspondence}.}:
