...
This says that the Bonacich reflective centrality at step $q$ is just the sum of the group centralities at step $q-1$ (for people) and the sum of the person centralities at step $q-1$ (for groups). 

Of course, these sums of sums would diverge to a larger and larger quantity at each step $q$. To prevent this and guarantee convergence, we normalize the vector of reflective Bonacich centralities for persons and groups at each step $q > 1$ before calculating the subsequent sum at step $q+1$ as follows:
