...
```{r Dual Projection CA}
   ...
   CA.r <- eigen(P.pp)
   CA.c <- eigen(P.gg)
   CA.r$values <- Re(CA.r$values)
   CA.r$vectors <- Re(CA.r$vectors)
   CA.c$values <- Re(CA.c$values)
   CA.c$vectors <- Re(CA.c$vectors)
...
```{r Weighted Similarity CA}
   ...
   SCA.r$values <- Re(SCA.r$values)
   SCA.r$vectors <- Re(SCA.r$vectors)
   SCA.c$values <- Re(SCA.c$values)
   SCA.c$vectors <- Re(SCA.c$vectors)
...
```{r Blocked Affiliation Matrices}
      # First CA Dimension
      A.ord <- A[order(CA.r$vectors[, 2]), order(CA.c$vectors[, 2])]
      ...
      A.ord <- A[order(eig.r[, 1]), order(eig.c[, 1])]
      p <- ggcorrplot(t(A.ord)) 
      p <- p + scale_x_discrete(position = "top")
      p <- p + scale_y_discrete(limits = rev)
     ...
     # Second Bonacich Dimension
      A.ord <- A[order(eig.r[, 2]), order(eig.c[, 2])]
      p <- ggcorrplot(t(A.ord)) 
      p <- p + scale_x_discrete(position = "top")
      p <- p + scale_y_discrete(limits = rev)
     ...
     # Second SimRank Eigenvector
      A.ord <- A[order(SR.eig.p[, 2]), order(SR.eig.g[, 2])]
      p <- ggcorrplot(t(A.ord)) 
      p <- p + scale_x_discrete(position = "top")
      p <- p + scale_y_discrete(limits = rev)
