...
```{r Faust-Style Direct CA}
   ca.res <- svd(diag(1/sqrt(rowSums(A))) %*% A %*% diag(1/sqrt(colSums(A))))
   d1.r <- ca.res$d[2] * ca.res$u[, 2] * sqrt(N/rowSums(A))
   d1.c <- ca.res$d[2] * ca.res$v[, 2] * sqrt(N/colSums(A))
   d1 <- c(d1.r, d1.c)
   d2.r <- ca.res$d[3] * ca.res$u[, 3] * sqrt(N/rowSums(A))
   d2.c <- ca.res$d[3] * ca.res$v[, 3] * sqrt(N/colSums(A))
   d2 <- c(d2.r, d2.c)
   d3.r <- ca.res$d[4] * ca.res$u[, 4] * sqrt(N/rowSums(A))
   d3.c <- ca.res$d[4] * ca.res$v[, 4] * sqrt(N/colSums(A))
   d3 <- c(d3.r, d3.c)
   d4.r <- ca.res$d[5] * ca.res$u[, 5] * sqrt(N/rowSums(A))
   d4.c <- ca.res$d[5] * ca.res$v[, 5] * sqrt(N/colSums(A))
   d4 <- c(d4.r, d4.c)
```
...
```{Two Dual Projection Eigenvector Scores}
...
```{r CA Eigenvector Plot} 
      ...
      eigvec.dat.c <- data.frame(rank = rank(d1.c), 
                             ...
                             lab = names(d1.c))
      eigval.dat.r <- data.frame(k = 1:nrow(A)-1, value = as.numeric(CA.r$values)) %>% 
             mutate(value = value / max(value))
      eigval.dat.c <- data.frame(k = 1:ncol(A)-1, value = as.numeric(CA.c$values)) %>% 
             mutate(value = value / max(value))
